...
自分の OS のものだけを上から順に貼る (各手順の末尾で折り畳んである「補足」の中のブロックは、
手順を進めるためには貼らなくてよい)。理由・実測・落とし穴は、手順ごとのものはその手順の
...
1. **パッケージマネージャを用意する**
   ...
   <details>
   <summary>補足: パッケージマネージャの選択</summary>
...
1. **Neovim 本体を入れる**
   ...
   Windows は手順 3 の `scoop install` で入っている。Linux は Homebrew で入れる
   (ディストリの Neovim は古すぎる)。
   ...
   **0.11.2 以上であることを確認する** (LazyVim の要求)。
   ...
   <details>
   <summary>補足: Neovim の入手経路</summary>
...
1. **設定を配置して初回起動する**
   ...
   **Neovim の設定ディレクトリそのものがこのリポジトリ**なので、clone 先を間違えないこと。
   ...
   if (-not $REPO_URL -or -not $NVIM_CONFIG) { throw "手順 1 のブロックを貼り直す" }
   ...
   git clone "${REPO_URL:?手順 1 の REPO_URL が空のまま。値を入れて貼り直す}" \
             "${NVIM_CONFIG:?手順 1 の NVIM_CONFIG が空のまま。値を入れて貼り直す}"
   nvim
   ```
   ...
   <details>
   <summary>補足: clone 先と <code>:Lazy restore</code></summary>
...
1. **動作確認する**
   ...
   <details>
   <summary>補足: headless での確認</summary>
...
手順 2〜6 で入れたコマンドは他でも使うので、消すかどうかは別に判断する
...
手順全体に関わる理由・実測・落とし穴 (手順ごとのものは各手順の末尾の「補足」にある)。
実行するだけなら読まなくてよい。
...
> **注記**: 環境固有の値は**シェル変数**で書いてある。[手順 1](#実施手順) で 1 度だけ
